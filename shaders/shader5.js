...
export const shader = {
    ...
    vec3 trans(vec3 p, inout float index_x, inout float index_y){
      index_x = mod(p.x / 6.0 , 6.0);
      index_y = mod(p.y / 6.0 , 6.0);
    ...
    float distanceFunc(vec3 p){
      ...
      p = abs(trans(p, index_x, index_y));

      if(index_x > sin(time)*2.5+3.5 || index_y > sin(time)*2.5+3.5){
        return 1.0;
      }
